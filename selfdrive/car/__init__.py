...
def create_button_enable_events(buttonEvents: capnp.lib.capnp._DynamicListBuilder,
                                enable_pressed: bool = False, enable_from_brake: bool = False,
                                mads_enabled: bool = False, cruise_state_enabled: bool = False) -> List[int]:
  events = []
  for b in buttonEvents:
    ...
    if b.type == ButtonType.altButton1 and b.pressed:
      if not mads_enabled:  # disabled MADS
        if not cruise_state_enabled:
          events.append(EventName.buttonCancel)
        else:
          events.append(EventName.manualSteeringRequired)
      else:  # enabled MADS
        if not cruise_state_enabled:
          enable_pressed = True
    ...
    if b.type == ButtonType.cancel and b.pressed:
      if not mads_enabled:
        ...
        events.append(EventName.manualLongitudinalRequired)
  if (cruise_state_enabled or mads_enabled) and enable_pressed:
    if enable_from_brake:
      events.append(EventName.silentButtonEnable)
    else:

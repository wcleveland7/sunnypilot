...
class CarController:
  ...
  def update(self, CC, CS):
    ...
    if not self.CP.openpilotLongitudinalControl:
      ...
      if not CC.cruiseControl.resume and CS.out.cruiseState.enabled and not CS.out.gasPressed:
        cruise_button = self.get_cruise_buttons(CS, self.v_cruise_kph_prev)
        if cruise_button is not None:
          can_sends.append(hondacan.spam_buttons_command(self.packer, cruise_button, self.CP.carFingerprint))

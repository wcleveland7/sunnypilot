...
class CarController:
  def __init__(self, dbc_name, CP, VM):
    ...
    self.acc_type = self.param_s.get("VwAccType")
  ...
  def update(self, CC, CS, ext_bus):
    ...
    if not self.CP.pcmCruiseSpeed and CS.gra_stock_values["COUNTER"] != self.gra_acc_counter_last:
      if not (CC.cruiseControl.cancel or CC.cruiseControl.resume) and not CS.out.gasPressed:
        cruise_button = self.get_cruise_buttons(CS, self.v_cruise_kph_prev)
        if cruise_button is not None:
          counter = (CS.gra_stock_values["COUNTER"] + 1) % 16
          if self.acc_type == 0:
            can_sends.append(self.CCS.create_acc_buttons_control(self.packer_pt, ext_bus, CS.gra_stock_values, counter,
                                                                 accel_cruise=(cruise_button == 1), decel_cruise=(cruise_button == 2)))
          elif self.acc_type == 1:
            can_sends.append(self.CCS.create_acc_buttons_control(self.packer_pt, ext_bus, CS.gra_stock_values, counter,
                                                                 resume_cruise=(cruise_button == 1), set_cruise=(cruise_button == 2)))
  ...
  def get_cruise_buttons_status(self, CS):
    if not CS.out.cruiseState.enabled or any(be.type in (ButtonType.accelCruise, ButtonType.resumeCruise, ButtonType.decelCruise, ButtonType.setCruise) for be in CS.button_events):
      self.timer = 40
    elif self.timer:
      self.timer -= 1
    else:

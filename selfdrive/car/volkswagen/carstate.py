  ...
  def update(self, pt_cp, cam_cp, ext_cp, trans_type):
    ...
    if ret.cruiseState.available:
      if not self.CP.pcmCruise or not self.CP.pcmCruiseSpeed:
        if any(be.type in (ButtonType.accelCruise, ButtonType.resumeCruise) for be in self.button_events):
          self.accEnabled = True
        if any(be.type in (ButtonType.decelCruise, ButtonType.setCruise) for be in self.button_events):
          self.accEnabled = True
      if self.enable_mads:

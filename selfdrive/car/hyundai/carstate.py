...
class CarState(CarStateBase):
  ...
  def update(self, cp, cp_cam):
    ...
    if ret.cruiseState.available:
      if not self.CP.pcmCruise or (not self.CP.pcmCruiseSpeed and self.pcm_enabled and not self.prev_pcm_enabled):
        if self.prev_cruise_buttons == 2: # SET-
          ...
          if self.cruise_buttons[-1] != 1:
            self.accEnabled = True
        elif not self.CP.pcmCruiseSpeed and self.pcm_enabled and not self.prev_pcm_enabled:
